...
import { resolveGitWorktreePath, resolveWorktreeT3Home } from "@azure/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@azure/shared/hostProcess";
...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".azure"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@azure/contracts",
    "--filter=@azure/web",
    "--filter=azure",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=azure", "dev"],
  "dev:web": ["run", "--filter=@azure/web", "dev"],
  "dev:desktop": ["run", "--filter=@azure/desktop", "--filter=@azure/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`AZURE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("AZURE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("AZURE_PORT_OFFSET"),
  devInstance: optionalStringConfig("AZURE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `AZURE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric AZURE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed AZURE_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .azure > ambient AZURE_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.AZURE_HOME = resolvedBaseDir;
    } else {
      delete output.AZURE_HOME;
    }
    ...
    // agent working inside Azure Code), these leak through and the child server
    ...
    if (!isDesktopMode) {
      output.AZURE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.AZURE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.AZURE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.AZURE_PORT = String(serverPort);
      ...
      delete output.AZURE_SINGLE_ORIGIN_DEV;
      delete output.AZURE_MODE;
      delete output.AZURE_NO_BROWSER;
      delete output.AZURE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.AZURE_HOST = host;
    }

    if (!isDesktopMode) {
      output.AZURE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.AZURE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.AZURE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.AZURE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.AZURE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.AZURE_MODE = "web";
      delete output.AZURE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`AZURE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["AZURE_PORT_OFFSET", "AZURE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.azure` — see @azure/shared/devHome for why this must
    // outrank an ambient AZURE_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.AZURE_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.AZURE_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.AZURE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          // a few chunk requests. Only defaulted, so AZURE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.AZURE_BUNDLED_DEV === undefined) {
            env.AZURE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Azure Code data directory; runtime state is stored under userdata (equivalent to AZURE_HOME). Inside a git worktree this defaults to that worktree's own .azure so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to AZURE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("AZURE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to AZURE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("AZURE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to AZURE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("AZURE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to AZURE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("AZURE_PORT")),
  ),
