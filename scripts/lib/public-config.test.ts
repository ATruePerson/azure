...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.AZURE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.AZURE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.AZURE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.AZURE_RELAY_URL).toBeUndefined();
    expect(env.VITE_AZURE_RELAY_URL).toBeUndefined();
    expect(env.AZURE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.AZURE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.AZURE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.AZURE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.AZURE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.AZURE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "AZURE_CLERK_PUBLISHABLE_KEY=pk_root\nAZURE_CLERK_JWT_TEMPLATE=template_root\nAZURE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nAZURE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "AZURE_CLERK_PUBLISHABLE_KEY=pk_local\nAZURE_CLERK_JWT_TEMPLATE=template_local\nAZURE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nAZURE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).AZURE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          AZURE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          AZURE_CLERK_JWT_TEMPLATE: "template_ci",
          AZURE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          AZURE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      AZURE_RELAY_URL: "https://ci.example.test",
      VITE_AZURE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        AZURE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_AZURE_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          AZURE_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          AZURE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          AZURE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          AZURE_RELAY_URL: "https://relay.example.test",
          AZURE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          AZURE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          AZURE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      AZURE_RELAY_URL: "https://relay.example.test",
      VITE_AZURE_RELAY_URL: "https://relay.example.test",
      AZURE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      AZURE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      AZURE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "azure-public-config-"));
