...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.azure.codes")),
    ).toBe("clerk.azure.codes");
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.azure.codes"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.azure.codes/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.azure.codes", "clerk.azure.codes")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.azure.codes")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.azure.codes", "clerk.azure.codes")).toBe(
      false,
    );
  });
});
