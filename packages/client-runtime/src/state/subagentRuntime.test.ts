...
import { classifyTaskAgentKind, type OrchestrationThreadActivity } from "@azure/contracts";
import {
...
describe("terminal robustness", () => {
  ...
  it("provider endedAt wins over ingestion time on the settling transition", () => {
    const agents = fold([
      activity("task.started", { taskId: "azure", taskType: "local_agent" }),
      activity(
        "task.updated",
        { taskId: "azure", status: "failed", endedAt: "2026-08-01T09:59:59.000Z" },
