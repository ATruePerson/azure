...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    AZURE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    AZURE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    AZURE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    AZURE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    AZURE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    AZURE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("AZURE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("AZURE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "AzureCodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Azure Code relay through Alchemy."));
