import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@azure/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "azure:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Azure Code version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx azure@${targetVersion}`;
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the Azure Code desktop app on its machine — update the desktop app there to sync them.`;
    default:
