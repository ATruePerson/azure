...
} from "@azure/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@azure/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@azure/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@azure/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@azure/client-runtime/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Azure Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Azure Connect session is unavailable.",
          });
        }
        return token;
      }),
