import type { PreviewOpenInput, PreviewSessionSnapshot, ScopedThreadRef } from "@azure/contracts";
...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(open).toHaveBeenCalledWith({ threadId: "thread-1", url: "azure.chat" });
    expect(readThreadPreviewState(threadRef).snapshot).toEqual(snapshot);
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://azure.chat/"]);
  });
