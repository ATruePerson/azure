...
import { connectionStatusText } from "@azure/client-runtime/connection";
import {
...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "Azure Connect" : null,
  ].filter((value): value is string => value !== null);
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Azure Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Azure Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Azure Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Azure Connect linked"
          : publishAgentActivity
            ? "Azure Connect tunnel disabled"
            : "Azure Connect unlinked",
        description: enabled
          ? "This environment is available through Azure Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Azure Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      <SettingsRow
        ...
        description={
          managedTunnelActive
            ? "This environment is available to your other devices through Azure Connect."
            : "Make this environment available to your other devices through Azure Connect."
        }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without an Azure Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Azure Connect."
