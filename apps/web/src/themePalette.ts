...
export const T3_CHAT_THEME_LABEL = "Azure Chat";
...
// genuinely Azure-branded palette keeps it. Stored preferences and mixes with the
...
// Older builds stored the dark Azure Chat palette as a separate theme. Keep
...
const T3_CHAT_LIGHT_COLORS: ThemeColors = {
  canvas: "#fdf7fd",
  // Azure Code's workspace header belongs to the chat panel, so keep it seamless
  // with the light chat canvas rather than mapping it to Azure Chat's outer shell.
  chrome: "#fdf7fd",
  toolbar: "#fdf7fd",
  toolbarForeground: "#501854",
  toolbarBorder: "#efbdeb",
  // Azure Chat's light chrome controls sit on its pale gradient-noise surface,
  ...
  // Azure Chat uses a light lavender code surface in light mode. Keeping the
  // dark plum pair here also leaked the dark palette into Azure Code's diffs.
  codeBackground: "#f5ecf9",
  codeForeground: "#673c8b",
  // The live sidebar is transparent over Azure Chat's outer shell. Use that
...
const T3_CHAT_DARK_COLORS: ThemeColors = {
  canvas: "#1f1a24",
  // Azure Code's workspace header belongs to the chat panel, so keep it seamless
  // with the canvas rather than mapping it to Azure Chat's outer shell.
  chrome: "#1f1a24",
  toolbar: "#1f1a24",
  ...
  // Cards and panels stay in Azure Chat's plum surface family. Near-black here
  ...
  // Diffs and file previews are full workspace surfaces in Azure Code. Keep them
  ...
  // Azure Chat draws the chat panel edge in this muted pink. The resize rail uses
...
/**
 * The palette Azure Code wears with no theme installed, captured from the app's
...
/**
 * The standard Azure Code look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Azure Chat palette used to fill roles omitted by theme
 * files.
 */
...
/**
 * The status colors Azure Code shows without a theme, read from the app's own
...
/**
 * A companion action color in the Azure Chat mold. This gives send buttons,
