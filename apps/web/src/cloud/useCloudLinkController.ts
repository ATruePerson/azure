...
import { findErrorTraceId } from "@azure/client-runtime/errors";
import {
...
/**
 * Drives the primary environment's Azure Connect link. Azure Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Azure Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Azure Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Azure Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Azure Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Azure Connect before enabling this."));
        return false;
      }
