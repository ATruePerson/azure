...
const ROUTER_HOST = "app.azure.codes";
const HOSTED_WEB_CHANNEL_COOKIE = "azure_web_channel";
const LATEST_ORIGIN = "https://latest.app.azure.codes";
const NIGHTLY_ORIGIN = "https://nightly.app.azure.codes";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @azure/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@azure/scripts...' --filter '@azure/web...'",
  routes: [
    {
      src: "/__azure/channel",
