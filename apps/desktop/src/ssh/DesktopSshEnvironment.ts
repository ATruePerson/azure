...
import * as SshAuth from "@azure/ssh/auth";
import { discoverSshHosts } from "@azure/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@azure/ssh/errors";
import * as SshTunnel from "@azure/ssh/tunnel";
...
>()("@azure/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Azure window is not available for SSH authentication.";
      break;
