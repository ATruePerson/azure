...
>()("@azure/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "AZURE_NO_BROWSER",
  "AZURE_HOST",
  "AZURE_DESKTOP_WS_URL",
  "AZURE_DESKTOP_LAN_ACCESS",
  "AZURE_DESKTOP_LAN_HOST",
  "AZURE_DESKTOP_HTTPS_ENDPOINTS",
  "AZURE_TAILSCALE_SERVE",
  "AZURE_TAILSCALE_SERVE_PORT",
] as const;
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's AZURE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // Build an explicit copy of process.env minus AZURE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.azure via
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus AZURE_HOME; pass it
