...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.legacyUserDataDirName, "Azure Code (Dev)");
      assert.deepEqual(
  ...
  it.effect("stores production state under userdata in an explicit home", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.legacyUserDataDirName, "Azure Code (Alpha)");
      assert.equal(environment.logDir, "/tmp/t3/userdata/logs");
      assert.equal(environment.browserArtifactsDir, "/tmp/t3/userdata/browser-artifacts");
      assert.equal(environment.serverSettingsPath, "/tmp/t3/userdata/settings.json");
    }),
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(production.legacyUserDataDirName, "Azure Code (Alpha)");
    }),
  );
