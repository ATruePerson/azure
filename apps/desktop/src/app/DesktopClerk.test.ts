...
describe("DesktopClerk", () => {
  ...
  it("derives the Clerk Frontend API hostname used by the desktop CSP", () => {
    const publishableKey = `pk_test_${btoa("clerk.azure.codes$")}`;
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "azure-code-dev", host: "app" },
          },
        ],
      ]);
  ...
  it.each([
    { isDevelopment: true, scheme: "azure-code-dev" },
    { isDevelopment: false, scheme: "azure-code" },
