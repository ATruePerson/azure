...
const AppPackageMetadata = Schema.Struct({
  azureCommitHash: Schema.optional(Schema.String),
});
...
>()("@azure/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.azureCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
