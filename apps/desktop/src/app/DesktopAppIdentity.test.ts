...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Azure Code"),
              ...
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true &&
                      path === "/Users/alice/Library/Application Support/Azure Code (Alpha)",
                  ),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"azureCommitHash":"abcdef1234567890"}'),
          }),
        ),
...
describe("DesktopAppIdentity", () => {
  it.effect("uses the legacy userData path for an old-only installation", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Azure Code (Alpha)");
      }),
      { legacyPathExists: true },
    ),
  );

  it.effect("uses the Azure userData path for a new-only installation", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/azure-code");
      }),
    ),
  );

  it.effect("keeps the legacy userData path when both installations exist", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/Azure Code (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Azure Code"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Azure Code");
