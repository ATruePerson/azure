...
const remoteDebuggingPort = process.env.AZURE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line azure/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.AZURE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.AZURE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--azure-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--azure-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
