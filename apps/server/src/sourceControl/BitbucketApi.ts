...
import { detectSourceControlProviderFromRemoteUrl } from "@azure/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("AZURE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("AZURE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("AZURE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("AZURE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("azure/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `azure/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set AZURE_BITBUCKET_EMAIL and AZURE_BITBUCKET_API_TOKEN, or AZURE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
