...
import { HostProcessArguments } from "@azure/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `azure` command.
 */
...
/**
 * The `azure` package spec to suggest. The literal spec the user typed (e.g.
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "t3@nightly" : "azure";
}

/**
 * Render a `t3 <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx azure connect` suggests
 * `npx azure serve`, a global install suggests `t3 serve`, and a nightly build
