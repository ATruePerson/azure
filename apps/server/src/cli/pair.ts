 ...
 * own `.azure` is checked first (matching dev-runner precedence); otherwise the
...
import { resolveWorktreeT3Home } from "@azure/shared/devHome";
import {
  buildTailscaleHttpsBaseUrl,
  DEFAULT_TAILSCALE_SERVE_PORT,
  ensureTailscaleServe,
  readTailscaleStatus,
} from "@azure/tailscale";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Azure Code server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx azure serve`, or connect this machine with T3 Connect: `npx azure connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Azure Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Azure Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.azure`
    ...
    const envHome = yield* Config.string("AZURE_HOME").pipe(Config.option);
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Azure Code server and print it as a QR code.",
  ),
