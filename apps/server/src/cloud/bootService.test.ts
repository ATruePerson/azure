...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.azure/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.azure",
    logPath: "/home/theo/.azure/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/azure.service",
  });

  expect(unit).toContain("ExecStart=/usr/bin/node /home/theo/.azure/runtime/service-launcher.mjs");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const baseDir = path.join(home, ".azure");
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("systemctl "))).toEqual([
        "systemctl --user stop azure.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart azure.service",
      ]);
    }),
  );
