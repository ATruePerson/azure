import { CONNECT_OAUTH_SCOPES, DEFAULT_HOSTED_APP_URL } from "@azure/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@azure/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@azure/shared/relayUrl";
...
declare const __AZURE_BUILD_RELAY_URL__: string | undefined;
declare const __AZURE_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __AZURE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __AZURE_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__AZURE_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __AZURE_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __AZURE_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __AZURE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __AZURE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __AZURE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.AZURE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.AZURE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.AZURE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("AZURE_RELAY_URL");
...
export function makeCloudCliOAuthConfig({
  ...
  return Config.all({
    clerkPublishableKey: makePublicValueConfig(
      "AZURE_CLERK_PUBLISHABLE_KEY",
      clerkPublishableKeyFallback,
    ),
    clientId: makePublicValueConfig(
      "AZURE_CLERK_CLI_OAUTH_CLIENT_ID",
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.AZURE_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.AZURE_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.AZURE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
