...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@azure/contracts";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".azure", "vcs.json");
