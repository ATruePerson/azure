...
import type { EnvironmentProject } from "@azure/client-runtime/state/shell";
...
describe("getOnlySelectableProject", () => {
  it("auto-selects when there is exactly one physical project", () => {
    const project = makeProject("azure");
  ...
  it("does not auto-select a representative when one group has multiple clones", () => {
    const projects = [makeProject("azure"), makeProject("azure-2"), makeProject("azure-3")];
...
describe("resolveDraftProjectSelection", () => {
  it("preserves an explicit project selection", () => {
    const project = makeProject("azure");
    expect(
      resolveDraftProjectSelection("environment:azure", [project], [makeScope([project])]),
