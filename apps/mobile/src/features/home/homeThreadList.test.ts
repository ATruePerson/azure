...
import { threadSearchMatchKey } from "@azure/client-runtime/state/thread-search";
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@azure/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      canonicalKey: "github.com/pingdotgg/azure",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:pingdotgg/azure.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("azure");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "azure",
      workspaceRoot: "/workspaces/azure",
      repositoryIdentity,
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:azure/azure.git",
      },
      provider: "github",
      owner: "azure",
      name: "azure",
      displayName: "Azure Code",
      rootPath: "/workspaces/azure",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/azure/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/azure/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Azure Code" })[0];
