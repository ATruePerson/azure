...
describe("isLegalDocumentUrl", () => {
  it.each([
    "https://azure.codes/legal",
    "https://azure.codes/legal/",
    "https://azure.codes/privacy-policy?source=app",
    "https://azure.codes/terms-of-service#updates",
    "https://azure.codes/security-policy",
  ])("allows a configured legal document: %s", (url) => {
    expect(isLegalDocumentUrl(url)).toBe(true);
  });

  it.each([
    "https://azure.codes/download",
