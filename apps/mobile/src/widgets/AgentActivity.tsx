...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `azure://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded Azure atom mark. `assetName` resolves the template image set
  // bundled in the widget extension's asset catalog. Image views only honor
  // `resizable` directly, so we size it via a container frame the resizable
  // image fills and tint it through the container's foreground style.
