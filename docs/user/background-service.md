# Running Azure Code in the Background

On a Linux host, Azure Code can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest Azure Code release:

```sh
npx azure@latest service install
```

Check whether it is installed:

```sh
npx azure@latest service status
```

Update or repair it:

```sh
npx azure@latest service update
```

Stop it and remove it from startup:

```sh
npx azure@latest service uninstall
```

Updating restarts Azure Code briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The systemd unit runs a small stable launcher. Exact Azure Code versions are installed separately, so
...
want Azure Code to start in the background.
