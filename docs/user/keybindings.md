...
The same configuration lives in `~/.azure/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Azure Code writes the built-in defaults into that file on
