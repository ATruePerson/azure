...
This guide is for people who want to use more than one Codex account in Azure Code. For ChatGPT Web,
see [ChatGPT Web](./providers-chatgpt-web.md). For Claude, see [Claude](./providers-claude.md). For
first-time setup, see [Install Azure Code](./install.md).
...
In Azure Code Settings, name it something obvious:
...
In Azure Code Settings, add another Codex provider:
...
Azure Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Azure Code treats it
