...
Use this when you want to connect to a Azure Code server from another device such as a phone, tablet, or separate desktop app.
...
`t3 pair` finds the running server (the shared `~/.azure` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `t3 pair` says so and points you at `npx azure serve` or `npx azure connect`.
...
run a headless server from the CLI, or have the desktop app launch Azure Code over SSH.
...
- HTTPS/WSS-compatible endpoints work from `https://app.azure.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.azure.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
For `https://app.azure.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
```bash
npx azure serve --host "$(tailscale ip -4)"
```
...
```bash
npx azure serve --tailscale-serve
```
...
```bash
npx azure serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse Azure Code on another machine over SSH.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.azure/ssh-launch/<host-key>/`, starts or reuses a remote T3 server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Azure Code uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Azure Code first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Azure Code uses:
...
If reconnecting after an app update fails, retry the SSH launch once. The launcher now compares its generated runner script, stops stale launcher-managed remote servers, clears the SSH launch PID/port state, and starts a fresh remote server. You should not normally need to delete `~/.azure/ssh-launch` or kill `azure` processes manually.

## Updating a Remote Server

When the Azure Code web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Azure Code may
...
see [Keeping Azure Code in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Azure Code in the Background](./background-service.md).
...
The hosted web app at `https://app.azure.codes` can save a remote backend in browser local storage from a URL like:

```text
https://app.azure.codes/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through Azure Code. The browser still connects directly to the backend URL in the pairing link.
