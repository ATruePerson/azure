...
```dotenv
AZURE_CLERK_PUBLISHABLE_KEY=<publishable key>
AZURE_CLERK_JWT_TEMPLATE=<JWT template name>
AZURE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
AZURE_RELAY_URL=https://relay.example.com
```
...
should set `AZURE_CLERK_PUBLISHABLE_KEY`, `AZURE_CLERK_JWT_TEMPLATE`,
`AZURE_CLERK_CLI_OAUTH_CLIENT_ID`, and `AZURE_RELAY_URL` before building. EAS preview and
...
deployment defaults.
`vp run --filter azure-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.azure.codes/connect/callback` for the hosted out-of-band flow. This is
     `connectCallbackUrl(DEFAULT_HOSTED_APP_URL)` from `packages/shared/src/connectAuth.ts`, so a
     custom `AZURE_HOSTED_APP_URL` means `$AZURE_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `AZURE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
Set `AZURE_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
...
is shared by production and non-production relay stages. The client-facing `AZURE_RELAY_URL` still
...
Local desktop development uses `azure-dev://app`, while packaged builds use `azure://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `azure-dev://app`; the production Clerk instance should only need
`azure://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.azure.azure`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.azure.azure` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.azure.azure`.
...
AZURE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/azure.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
AZURE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `AZURE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`AZURE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `AZURE_PORT` set. Rebuild the signed app
...
AZURE_PORT=13773 \
  "/Applications/Azure Code (Alpha).app/Contents/MacOS/Azure Code (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Azure Code (Alpha).app"
codesign -d --entitlements :- "/Applications/Azure Code (Alpha).app"
```
