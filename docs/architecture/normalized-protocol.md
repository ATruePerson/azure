...
- `previous_response_id` is resolved against an encrypted, bounded local response store. ACC does not upload conversation state to implement continuity.
...
The response store is local, encrypted, bounded to 100 entries, and expires entries after 24 hours. Its key and ciphertext live under the ACC config root with mode `0600`; the direct Codex gateway remains bound to `127.0.0.1`; no second compatibility proxy is required.
