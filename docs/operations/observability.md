...
Azure Code has one server-side observability model:
...
stdout/stderr at `~/.azure/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.azure/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.azure/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.azure/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.azure/dev/logs/server.trace.ndjson`.
...
```bash
export AZURE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export AZURE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export AZURE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export AZURE_TRACE_MIN_LEVEL=Info
export AZURE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `AZURE_OTLP_*`.

macOS app bundle example:

```bash
AZURE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
AZURE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
AZURE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Azure Code.app/Contents/MacOS/Azure Code"
```
...
```bash
TRACE_FILE="${AZURE_HOME:-$HOME/.azure}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.azure/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.azure/dev/logs/server.trace.ndjson"
```
...
- `AZURE_OTLP_TRACES_URL` was not set
...
- `AZURE_TRACE_FILE`: override trace file path
- `AZURE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `AZURE_TRACE_MAX_FILES`: rotated file count, default `10`
- `AZURE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `AZURE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `AZURE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `AZURE_OTLP_TRACES_URL`: OTLP trace endpoint
- `AZURE_OTLP_METRICS_URL`: OTLP metric endpoint
- `AZURE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `AZURE_OTLP_SERVICE_NAME`: service name, default `t3-server`
